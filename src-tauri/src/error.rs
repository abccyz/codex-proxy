...
impl IntoResponse for ProxyError {
    fn into_response(self) -> Response {
        let (status, error_type) = match &self {
            ...
            ProxyError::UpstreamError(_) | ProxyError::UpstreamHttp(_, _) => (StatusCode::BAD_GATEWAY, "upstream_error"),
            ProxyError::InvalidUpstreamResponse(_) | ProxyError::Internal(_) => (StatusCode::BAD_GATEWAY, "proxy_error"),
            ProxyError::Request(_) => (StatusCode::BAD_GATEWAY, "proxy_error"),
        };
        let body = serde_json::json!({ "error": { "message": self.to_string(), "type": error_type } });
